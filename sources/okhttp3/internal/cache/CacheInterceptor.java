...
public final class CacheInterceptor implements Interceptor {
    ...
    public final Response intercept(Interceptor.Chain chain) throws IOException {
        ...
        if (request == null && response2 == null) {
            return new Response.Builder().request(chain.request()).protocol(Protocol.HTTP_1_1).code(SecExceptionCode.SEC_ERROR_DYN_STORE_GET_DATA_FILE_KEY_FAILED).message("Unsatisfiable Request (only-if-cached)").body(Util.EMPTY_RESPONSE).sentRequestAtMillis(-1L).receivedResponseAtMillis(System.currentTimeMillis()).build();
        }
        if (request == null) {
    ...
    private Response cacheWritingResponse(final CacheRequest cacheRequest, Response response) throws IOException {
        ...
        final BufferedSink BuiltInFictitiousFunctionClassFactory = Okio.BuiltInFictitiousFunctionClassFactory(body);
        return response.newBuilder().body(new RealResponseBody(response.header("Content-Type"), response.body().contentLength(), Okio.KClassImpl$Data$declaredNonStaticMembers$2(new Source() { // from class: okhttp3.internal.cache.CacheInterceptor.1
            ...
            public long read(Buffer buffer, long j) throws IOException {
                try {
                    ...
                    buffer.BuiltInFictitiousFunctionClassFactory(BuiltInFictitiousFunctionClassFactory.KClassImpl$Data$declaredNonStaticMembers$2(), buffer.getAuthRequestContext - read, read);
    ...
    static boolean isContentSpecificHeader(String str) {
        return com.google.common.net.HttpHeaders.CONTENT_LENGTH.equalsIgnoreCase(str) || "Content-Encoding".equalsIgnoreCase(str) || "Content-Type".equalsIgnoreCase(str);
    }
}
