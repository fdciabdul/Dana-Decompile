...
public class ConstraintProxyUpdateReceiver extends BroadcastReceiver {
    ...
    static final String TAG = Logger.MyBillsEntityDataFactory("ConstrntProxyUpdtRecvr");
    ...
    public void onReceive(final Context context, final Intent intent) {
        ...
        if (!ACTION.equals(action)) {
            Logger.getAuthRequestContext().PlaceComponentResult(TAG, String.format("Ignoring unknown action %s", action), new Throwable[0]);
            return;
        }
        final BroadcastReceiver.PendingResult goAsync = goAsync();
        WorkManagerImpl.PlaceComponentResult(context).getErrorConfigVersion.getAuthRequestContext(new Runnable() { // from class: androidx.work.impl.background.systemalarm.ConstraintProxyUpdateReceiver.1
            ...
            public void run() {
                try {
                    ...
                    Logger.getAuthRequestContext().PlaceComponentResult(ConstraintProxyUpdateReceiver.TAG, String.format("Updating proxies: BatteryNotLowProxy enabled (%s), BatteryChargingProxy enabled (%s), StorageNotLowProxy (%s), NetworkStateProxy enabled (%s)", Boolean.valueOf(booleanExtra), Boolean.valueOf(booleanExtra2), Boolean.valueOf(booleanExtra3), Boolean.valueOf(booleanExtra4)), new Throwable[0]);
                    PackageManagerHelper.PlaceComponentResult(context, ConstraintProxy.BatteryNotLowProxy.class, booleanExtra);
                    PackageManagerHelper.PlaceComponentResult(context, ConstraintProxy.BatteryChargingProxy.class, booleanExtra2);
                    PackageManagerHelper.PlaceComponentResult(context, ConstraintProxy.StorageNotLowProxy.class, booleanExtra3);
                    PackageManagerHelper.PlaceComponentResult(context, ConstraintProxy.NetworkStateProxy.class, booleanExtra4);
                } finally {
                    goAsync.finish();
                }
            }
