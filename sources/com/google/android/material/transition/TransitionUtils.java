...
class TransitionUtils {
    ...
    static TimeInterpolator resolveThemeInterpolator(Context context, int i, TimeInterpolator timeInterpolator) {
        ...
        if (context.getTheme().resolveAttribute(i, typedValue, true)) {
            ...
            if (isEasingType(valueOf, EASING_TYPE_CUBIC_BEZIER)) {
                ...
                return PathInterpolatorCompat.KClassImpl$Data$declaredNonStaticMembers$2(getControlPoint(split, 0), getControlPoint(split, 1), getControlPoint(split, 2), getControlPoint(split, 3));
            } else if (isEasingType(valueOf, "path")) {
                return PathInterpolatorCompat.PlaceComponentResult(PathParser.MyBillsEntityDataFactory(getEasingContent(valueOf, "path")));
            } else {
    ...
    static PathMotion resolveThemePath(Context context, int i) {
        ...
        if (context.getTheme().resolveAttribute(i, typedValue, true)) {
            if (typedValue.type == 16) {
                ...
                return new PatternPathMotion(PathParser.MyBillsEntityDataFactory(String.valueOf(typedValue.string)));
            } else {
    ...
    public static ShapeAppearanceModel convertToRelativeCornerSizes(ShapeAppearanceModel shapeAppearanceModel, final RectF rectF) {
        return shapeAppearanceModel.withTransformedCornerSizes(new ShapeAppearanceModel.CornerSizeUnaryOperator() { // from class: com.google.android.material.transition.TransitionUtils.1
            ...
            public final CornerSize apply(CornerSize cornerSize) {
                return !(cornerSize instanceof RelativeCornerSize) ? new RelativeCornerSize(cornerSize.getCornerSize(rectF) / rectF.height()) : cornerSize;
            }
        });
    }
    ...
    static void maybeRemoveTransition(TransitionSet transitionSet, Transition transition) {
        if (transition != null) {
            transitionSet.getAuthRequestContext.remove(transition);
