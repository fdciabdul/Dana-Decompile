    ...
    public final void getAuthRequestContext(Subscriber<? super R>[] subscriberArr) {
        if (MyBillsEntityDataFactory(subscriberArr)) {
            ...
            for (int i = 0; i < length; i++) {
                subscriberArr2[i] = FlowableConcatMap.PlaceComponentResult(subscriberArr[i], this.getAuthRequestContext, this.MyBillsEntityDataFactory, this.PlaceComponentResult);
            }
