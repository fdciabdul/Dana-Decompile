...
public class Platform {
    ...
    public X509TrustManager trustManager(SSLSocketFactory sSLSocketFactory) {
        try {
            Object readFieldOrNull = readFieldOrNull(sSLSocketFactory, Class.forName("sun.security.ssl.SSLContextImpl"), HummerConstants.CONTEXT);
    ...
    public static byte[] concatLengthPrefixed(List<Protocol> list) {
        ...
        for (int i = 0; i < size; i++) {
            Protocol protocol = list.get(i);
            if (protocol != Protocol.HTTP_1_0) {
                buffer.PlaceComponentResult(protocol.toString().length());
                String obj = protocol.toString();
                buffer.PlaceComponentResult(obj, 0, obj.length());
            }
        }
        return buffer.NetworkUserEntityData$$ExternalSyntheticLambda1();
    }
