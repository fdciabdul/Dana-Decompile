...
public class GriverV8ImportJSErrorExtension implements NodeAware<App>, V8ImportScriptErrorPoint {
    ...
    public void onLoadScriptError(Throwable th) {
        ...
        GriverMonitor.error(GriverMonitorConstants.ERROR_JS_EXECUTE_EXCEPTION, "GriverAppContainer", new MonitorMap.Builder().appId(app.getAppId()).version((AppModel) app.getData(AppModel.class)).needAsynAppType(true).exception(th).workerType(GriverMonitorConstants.WORKER_TYPE_V8).message(GriverMonitorConstants.MESSAGE_WORKER_LOAD_SCRIPT_EXCEPTION).build());
    }
