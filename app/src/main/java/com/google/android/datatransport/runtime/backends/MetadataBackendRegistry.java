    ...
    static class BackendFactoryProvider {
        ...
        BackendFactory get(String str) {
            ...
            try {
                ...
                InstrumentInjector.log_w(MetadataBackendRegistry.TAG, String.format("Class %s is not found.", str2), e);
                return null;
            } catch (IllegalAccessException e2) {
                InstrumentInjector.log_w(MetadataBackendRegistry.TAG, String.format("Could not instantiate %s.", str2), e2);
                return null;
            } catch (InstantiationException e3) {
                InstrumentInjector.log_w(MetadataBackendRegistry.TAG, String.format("Could not instantiate %s.", str2), e3);
                return null;
            } catch (NoSuchMethodException e4) {
                InstrumentInjector.log_w(MetadataBackendRegistry.TAG, String.format("Could not instantiate %s", str2), e4);
                return null;
            } catch (InvocationTargetException e5) {
                InstrumentInjector.log_w(MetadataBackendRegistry.TAG, String.format("Could not instantiate %s", str2), e5);
                return null;
            }
        }
        ...
        private Map<String, String> discover(Context context) {
            ...
            if (metadata == null) {
                InstrumentInjector.log_w(MetadataBackendRegistry.TAG, "Could not retrieve metadata, returning empty list of transport backends.");
        ...
        private static Bundle getMetadata(Context context) {
            try {
                ...
                if (packageManager == null) {
                    InstrumentInjector.log_w(MetadataBackendRegistry.TAG, "Context has no PackageManager.");
                    return null;
                }
                ServiceInfo serviceInfo = packageManager.getServiceInfo(new ComponentName(context, TransportBackendDiscovery.class), 128);
                if (serviceInfo == null) {
                    InstrumentInjector.log_w(MetadataBackendRegistry.TAG, "TransportBackendDiscovery has no service info.");
                    return null;
                }
                return serviceInfo.metaData;
            } catch (PackageManager.NameNotFoundException unused) {
                InstrumentInjector.log_w(MetadataBackendRegistry.TAG, "Application info not found.");
                return null;
            }
        }
    }
