...
public class IidStore {
    ...
    private static String getIdFromPublicKey(PublicKey publicKey) {
        try {
            ...
            InstrumentInjector.log_w("ContentValues", "Unexpected error, device missing required algorithms");
            return null;
        }
    }
    ...
    private PublicKey parseKey(String str) {
        try {
            return KeyFactory.getInstance(RSAHelper.DEFAULT_ALGORITHM).generatePublic(new X509EncodedKeySpec(Base64.decode(str, 8)));
        } catch (IllegalArgumentException | NoSuchAlgorithmException | InvalidKeySpecException e) {
            StringBuilder sb = new StringBuilder();
            sb.append("Invalid key stored ");
            sb.append(e);
            InstrumentInjector.log_w("ContentValues", sb.toString());
            return null;
        }
    }
}
