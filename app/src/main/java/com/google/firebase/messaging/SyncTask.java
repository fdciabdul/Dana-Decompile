    ...
    boolean maybeRefreshToken() throws IOException {
        try {
            if (this.firebaseMessaging.blockingGetToken() == null) {
                InstrumentInjector.log_e(Constants.TAG, "Token retrieval failed: null");
                return false;
            } else if (Log.isLoggable(Constants.TAG, 3)) {
                InstrumentInjector.log_d(Constants.TAG, "Token successfully retrieved");
                return true;
            } else {
                return true;
            }
            ...
            if (GmsRpc.isErrorMessageForRetryableError(e.getMessage())) {
                ...
                InstrumentInjector.log_w(Constants.TAG, "Token retrieval failed without exception message. Will retry token retrieval");
                return false;
            } else {
                throw e;
            }
        } catch (SecurityException unused) {
            InstrumentInjector.log_w(Constants.TAG, "Token retrieval failed with SecurityException. Will retry token retrieval");
            return false;
        }
    }
    ...
    static class ConnectivityChangeReceiver extends BroadcastReceiver {
        ...
        public void registerReceiver() {
            if (SyncTask.isDebugLogEnabled()) {
                InstrumentInjector.log_d(Constants.TAG, "Connectivity change received registered");
            }
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (syncTask != null && syncTask.isDeviceConnected()) {
                if (SyncTask.isDebugLogEnabled()) {
                    InstrumentInjector.log_d(Constants.TAG, "Connectivity changed. Starting background sync.");
                }
