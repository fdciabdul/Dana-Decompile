...
public class StopOfflinePay extends SynchronizedUseCase<Boolean, Void> {
    private final QrPayRepository qrPayRepository;

    @Inject
    public StopOfflinePay(QrPayRepository qrPayRepository) {
    ...
    public Boolean buildUseCaseObservable(Void r1) {
        return this.qrPayRepository.stop();
    }
}
