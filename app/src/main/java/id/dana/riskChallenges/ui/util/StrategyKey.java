package id.dana.riskChallenges.ui.util;

import java.lang.annotation.Retention;
import java.lang.annotation.RetentionPolicy;
import kotlin.Metadata;
import kotlin.annotation.AnnotationRetention;

@Retention(RetentionPolicy.SOURCE)
@Metadata(d1 = {"\u0000\n\n\u0002\u0018\u0002\n\u0002\u0010\u001b\n\u0000\b\u0087\u0002\u0018\u0000 \u00022\u00020\u0001:\u0001\u0002B\u0000"}, d2 = {"Lid/dana/riskChallenges/ui/util/StrategyKey;", "", "Companion"}, k = 1, mv = {1, 6, 0}, xi = 48)
@kotlin.annotation.Retention(AnnotationRetention.SOURCE)
/* loaded from: classes3.dex */
public @interface StrategyKey {
    public static final String CREATE_PIN = "CREATE_PIN";

    /* renamed from: Companion  reason: from kotlin metadata */
    public static final Companion INSTANCE = Companion.BuiltInFictitiousFunctionClassFactory;
    public static final String PASSKEY_AUTHENTICATION = "PASSKEY_AUTHENTICATION";
    public static final String PASSKEY_REGISTRATION = "PASSKEY_REGISTRATION";
    public static final String PASSKEY_UNLINK = "PASSKEY_UNLINK";
    public static final String RESET_PIN = "RESET_PIN";
    public static final String SUSPICIOUS_ACTIVITY_VERIFICATION = "SUSPICIOUS_ACTIVITY_VERIFICATION";
    public static final String VERIFY_PIN = "VERIFY_PIN";

    @Metadata(d1 = {"\u0000\f\n\u0002\u0018\u0002\n\u0002\u0010\u0000\n\u0002\b\u0002\b\u0086\u0003\u0018\u00002\u00020\u0001B\t\b\u0002¢\u0006\u0004\b\u0002\u0010\u0003"}, d2 = {"Lid/dana/riskChallenges/ui/util/StrategyKey$Companion;", "", "<init>", "()V"}, k = 1, mv = {1, 6, 0}, xi = 48)
    /* loaded from: classes3.dex */
    public static final class Companion {
        static final /* synthetic */ Companion BuiltInFictitiousFunctionClassFactory = new Companion();

        private Companion() {
        }
    }
}
